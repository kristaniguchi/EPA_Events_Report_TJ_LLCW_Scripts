...
###############################################################################################################
#Read in the tables generated from Table2.3_cal_generate.R and all of the events scripts
...
total.q.obs.TCM = (table.obs$total.q.obs.mm/1000*10231811.9)/1000 #from mm to m3 to thousand m3
...
table.obs.df = data.frame(as.character(table.obs[,1]), as.numeric(table.obs[,2]), as.numeric(table.obs[,3]), as.numeric(table.obs[,4]), as.numeric(total.q.obs.TCM), as.numeric(table.obs[,7]), as.character(table.obs[,5]),  as.character(table.obs[,6])) #in order to round, must be dataframe, create fake dataframe to round

#Round the values 
round1 = txtRound(table.obs.df[,2:6], 2) #round the numeric columns to 2 digits, use round1[,1:4] and round1[,5]

#Table!
table.2.1.obs = data.frame(cbind(as.character(table.obs.df[,1]), round1[,1:4], as.numeric(round1[,5]), as.character(table.obs.df[,7]), as.character(table.obs.df[,8])))
names(table.2.1.obs) <- c("Event Date", "Total Precip. (mm)", "Peak.q.obs.cms", "Total.q.obs.mm", "Total.q.obs.TCM","runoff.coeff", "event", "source")
...
table.2.1.obs.tableout = htmlTable(table.2.1.obs, 
                                   ...
                                   header = c("", "", "Obs", "Obs (mm)", "Obs (TCM)", "Obs","", ""),
                                   ...
                                   caption="Table 2.3 Summary of storm events defined in Table 2.1, final observed data (obs) used for calibration/validation.")
